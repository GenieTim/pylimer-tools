...
namespace calc {
  namespace mehp {
    ...
    public:
      ...
      {
        INVALIDARG_EXP_IFN(
          igraph_cattribute_VAN(&this->graph, "type", vertexId) !=
            this->splipLinkType,
      ...
      size_t getOtherRailEdgeId(size_t vertexId, size_t railEdge)
      {
        ...
        igraph_vector_int_init(&parents, igraph_vector_int_size(&edgeIds));
        igraph_cattribute_EANV(&this->graph, "parent_edge", selector, &parents);

        igraph_int_t railParent = igraph_vector_int_get(&parents, rail_idx);
        ...
        std::vector<size_t> results;
        for (size_t i = 0; i < igraph_vector_int_size(&parents); ++i) {
          if ((igraph_vector_int_get(&parents, i) == railParent) &&
              (i != rail_idx)) {
        ...
        igraph_vector_int_t parentEdges;
        igraph_vector_int_init(&parentEdges, igraph_ecount(&this->graph));
        igraph_cattribute_EANV(&this->graph,
                               "parent_edge",
                               igraph_ess_all(IGRAPH_EDGEORDER_ID),
                               &parentEdges);
        igraph_vector_int_t allEdgesOfParent;
        igraph_vector_int_init(&allEdgesOfParent, 4);
        for (size_t i = 0; i < igraph_ecount(&this->graph); ++i) {
          if (igraph_vector_int_get(&parentEdges, i) == railParent) {
            ...
            igraph_cattribute_VAN_set(
              &this->graph, "prev_vertex_id", from, from);
        ...
        igraph_subgraph_from_edges(
          &this->graph, &subgraph, igraph_vss_vector(&allEdgesOfParent), true);
        ...
        for (size_t i = 0; i < igraph_vector_int_size(&edgesOnPath); ++i) {
          if (igraph_cattribute_EAN(&subgraph, "prev_edge_id", i) == railEdge) {
            ...
            if ((igraph_cattribute_VAN(&subgraph, "prev_vertex_id", from) ==
              ...
              igraph_edge(&subgraph, i + 1, &from, &to);
              assert((igraph_cattribute_VAN(
                        &subgraph, "prev_vertex_id", from) == vertexId) ||
                     (igraph_cattribute_VAN(&subgraph, "prev_vertex_id", to) ==
                      vertexId));
              // i+1 is on-rail
              result = igraph_cattribute_EAN(&subgraph, "prev_edge_id", i + 1);
