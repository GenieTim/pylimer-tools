...
namespace calc {
  namespace mehp {
                                               ...
                                               double constraintTol)
    {
      ...
      nlopt::func objectiveF =
        ...
        size_t nrOfPartitionParameters =
          net->nrOfPartialSprings - net->nrOfSprings;
        RUNTIME_EXP_IFN(n == (3 * net->nrOfLinks) + (nrOfPartitionParameters),
                        "Value of n does not match expectation, got " +
                          std::to_string(n) + " instead of 3 * " +
        ...
        return MEHPForceBalance2::evaluateForceSetGradient(net, u, grad);
      };
      ...
      if (nrOfSpringsRequiringParameter > 0) {
        nlopt::mfunc nonlinearConstraintF = [](unsigned m,
          ...
          size_t nrOfPartitionParameters =
            net->nrOfPartialSprings - net->nrOfSprings;
          RUNTIME_EXP_IFN(n == (3 * net->nrOfLinks) + (nrOfPartitionParameters),
                          "Constraint dimensions do not match expectation");
          size_t nrOfConstraints = net->nrOfSpringsWithPartition;
          RUNTIME_EXP_IFN(nrOfConstraints == m,
                          "Constraint dimensions do not match expectation");

          return MEHPForceBalance2::evaluateSetConstraintsAndGradients(
            net, u, result, grad);
        };
      ...
      assert(u0.size() == (3 * net.nrOfLinks) + nrOfPartialSpringParameters);
      ...
      std::cout << this->nrOfStepsDone << " steps done." << std::endl;
    }
     ...
     * @return std::vector<Eigen::ArrayXi>
     */
    std::vector<Eigen::ArrayXi> MEHPForceBalance2::getIndependentCoordinateSets(
      ForceBalanceNetwork* net) const
    {
      std::vector<Eigen::ArrayXi> results;
      // build graph
     ...
     *
     */
    double MEHPForceBalance2::updateSpringPartition(
        ...
        for (size_t partner_idx = 1; partner_idx < springsPartners.size() - 1;
          ...
          if (springsPartners[partner_idx] == linkIdx) {
            ...
            Eigen::Vector3d vecBack =
              (MEHPForceBalance2::evaluateDistanceBetween(
                net,
                u,
      ...
      for (size_t spring_index = 0; spring_index < springIndices.size();
        ...
        for (size_t partner_idx = 0; partner_idx < springsPartners.size() - 1;
          ...
          if (springsPartners[partner_idx] == linkIdx ||
            ...
            if (springsPartners[partner_idx] == linkIdx) {
              partialDistance = MEHPForceBalance2::evaluateDistanceBetween(
                net,
                u,
                springsPartners[partner_idx + 1],
                springsPartners[partner_idx],
      ...
      const double oneOverSpringPartitionUpperLimit) const
    {
      ...
      for (size_t springIndex : springIndices) {
        ...
        for (size_t partner_idx : partnerIndices) {
          ...
          if (partner_idx == 0) {
            distance = (MEHPForceBalance2::evaluateDistanceBetween(
    ...
    Eigen::Vector3d MEHPForceBalance2::evaluateDistanceBetween(
      const ForceBalanceNetwork* net,
      const Eigen::VectorXd& u,
    ...
    void MEHPForceBalance2::addSlipLinks(const std::vector<size_t> strandIdx1,
    ...
    std::array<std::array<double, 3>, 3> MEHPForceBalance2::getStressTensor(
      const double oneOverSpringPartitionUpperLimit) const
    {
    ...
    std::array<std::array<double, 3>, 3>
    MEHPForceBalance2::getStressTensorLinkBased(
      const double oneOverSpringPartitionUpperLimit,
      const bool xlinksOnly) const
    {
      ...
      bool usePartial) const
    {
      ...
      Eigen::VectorXi nrOfActiveSpringsConnected =
        usePartial ? this->getNrOfActivePartialSpringsConnected(tolerance)
                   : this->getNrOfActiveSpringsConnected(tolerance);
      for (size_t i = 0; i < this->initialConfig.nrOfNodes; i++) {
     ...
     * @return double
     */
    double MEHPForceBalance2::getGammaFactor(double r02, int nrOfChains) const
    {
      if (r02 < 0) {
    ...
    bool MEHPForceBalance2::validateNetwork(const ForceBalanceNetwork* net)
    {
      if (net == nullptr) {
      ...
      for (size_t i = 0; i < net->nrOfSprings; ++i) {
        ...
        for (size_t globalIdx : globalSpringIndices) {
          ...
          if (!net->linkIsSliplink[net->springPartIndexB[globalIdx]]) {
            nrOfBsFound += 1;
          }
          if (!net->linkIsSliplink[net->springPartIndexA[globalIdx]]) {
            nrOfAsFound += 1;
          }
        }
        RUNTIME_EXP_IFN(
          nrOfBsFound == 1,
          "Expected 1 spring part index B to be cross-link, got " +
            std::to_string(nrOfBsFound) + " in spring " + std::to_string(i) +
            ".");
        RUNTIME_EXP_IFN(
          nrOfAsFound == 1,
          "Expected 1 spring part index A to be cross-link, got " +
            std::to_string(nrOfAsFound) + " in spring " + std::to_string(i) +
            ".");
