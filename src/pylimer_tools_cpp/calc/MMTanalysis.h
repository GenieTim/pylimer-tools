...
Arguments:
  ...
  - junctionType: the type of the junctions/crosslinkers to select them in the
network
...
ignoring the crosslinkers.
  - functionalityPerType: a dictionary with key: type, and value: functionality
of this atom type. If `None`: will use max functionality per type.

Returns:
  - r (float): the stoichiometric inbalance
*/
double computeStoichiometricInbalance(pylimer_tools::entities::Universe network,
                                      int junctionType, int strandLength,
...
NOTE: if your system has a non-integer number of possible bonds (e.g. one site
unbonded), this will not be rounded/respected in any way.
