    ...
    while (!vertexStack.empty()) {
      ...
      if ((oldVertexIdToNewLinkId[currentVertex] == -1)) {
        ...
        if (vertexStack.empty()) {
          ...
          if (!addNewLink) {
            ...
            oldVertexIdToNewLinkId[currentVertex] =
              std::max(oldVertexIdToNewLinkId[fst],
        ...
        if (addSpring) {
          ...
          springIndicesOfStrand[newStrandId].push_back(
            springFrom.size());
      ...
      for (igraph_integer_t i = 0; i < igraph_vector_int_size(neighbors); ++i) {
        ...
        igraph_edge(graph.get(), edge, &a, &b);
        ...
        if (vertex_visited[neighbor] && edge_followed[edge]) {
          ...
          springIndicesOfStrand[newStrandId].push_back(springFrom.size());
    ...
    if (net.strandIndicesOfLink[crosslinkIdx].size() == 0 // f = 0
      ...
      (XOR(
        ...
        pylimer_tools::utils::last(
          net
          .linkIndicesOfStrand[net.strandIndicesOfLink[crosslinkIdx][0]]) ==
        crosslinkIdx))) {
#ifdef DEBUG_REMOVAL
                                ...
                                bool skipEigenResize) const
{
  ...
  int removed = 0;
  for (int j = net.linkIndicesOfStrand[fullSpringIdx].size() - 1; j >= 0;
       --j) {
                                ...
                                const size_t linkToReduce) const
{
  ...
  for (size_t linkOfRemovedSpring : removedSpringsLinks) {
    for (size_t i = 0; i < net.strandIndicesOfLink[linkOfRemovedSpring].size();
         ++i) {
      if (net.strandIndicesOfLink[linkOfRemovedSpring][i] ==
          removedStrandIdx) {
  ...
  for (size_t i = 0; i < net.springIndicesOfStrand[newKeptSpringIdx].size();
    ...
    std::vector<size_t> linkIndices =
      net.linkIndicesOfStrand[newKeptSpringIdx];
                                               ...
                                               const double tolerance) const
{
  ...
  Eigen::ArrayXd allActiveAtomsPerChains =
    activeSprings.cast<double>() * (net->springContourLength.array() -
                                   ...
                                   const Eigen::VectorXd& u) const
{
  ...
  for (size_t slipLinkIdx = net.nrOfNodes; slipLinkIdx < net.nrOfLinks;
       ++slipLinkIdx) {
    RUNTIME_EXP_IFN(
      net.strandIndicesOfLink[slipLinkIdx].size() == 2 ||
      net.strandIndicesOfLink[slipLinkIdx].size() == 1,
      "Expect each slip-link to be involved in exactly one or two "
      "springs, "
      "got " +
      std::to_string(net.strandIndicesOfLink[slipLinkIdx].size()) + ".");
