    ...
    void MEHPForceBalance2::runForceRelaxation(
        ...
        do {
            ...
            for (size_t springIdx = 0;
                 springIdx < this->initialConfig.nrOfSprings;
                 ++springIdx) {
                if (this->initialConfig.springIndexA[springIdx] ==
                    this->initialConfig.springIndexB[springIdx]) {
                    continue;
                }
                double oneOverContourLengthFraction = 1.0 / (this->initialConfig.springsContourLength[springIdx]);
                double multiplier =
                        this->kappa *
                ...
                diagonal.segment(3 * this->initialConfig.springIndexA[springIdx],
                                 3) -= Eigen::Vector3d::Constant(multiplier);
                diagonal.segment(3 * this->initialConfig.springIndexB[springIdx],
                                 3) -= Eigen::Vector3d::Constant(multiplier);

                // the constants, b in Ax = b
                constants.segment(3 * this->initialConfig.springIndexA[springIdx],
                                  3) -=
                        this->initialConfig.springBoxOffset.segment(3 * springIdx, 3) *
                        multiplier;
                constants.segment(3 * this->initialConfig.springIndexB[springIdx],
                                  3) +=
                        this->initialConfig.springBoxOffset.segment(3 * springIdx, 3) *
                        multiplier;
            }
...
#define SOLVE_DIRECT(solver)                                                   \
  ...
  break;
     ...
     * @return double
     */
    double MEHPForceBalance2::getDisplacementResidualNorm() const {
        Eigen::VectorXd oneOverSpringPartitions =
                this->assembleOneOverSpringPartition(this->initialConfig
                );
    ...
    double MEHPForceBalance2::getDisplacementResidualNormFor(
        ...
        Eigen::ArrayXd loopPartialSpringEliminator =
                (net.springCoordinateIndexA != net.springCoordinateIndexB)
                .cast<double>();
    ...
    Eigen::VectorXd MEHPForceBalance2::assembleOneOverSpringPartition(
        ...
        for (size_t i = 0; i < net.nrOfSprings; ++i) {
            double valueToSet =
                    1.0 / (net.springsContourLength[i]);
    ...
    size_t MEHPForceBalance2::removePrimaryLoops(
        ...
        size_t numRemoved = 0;
        for (long int springIdx = net.nrOfStrands - 1; springIdx >= 0;
             --springIdx) {
            if (net.linkIndicesOfStrands[springIdx][0] == pylimer_tools::utils::last(
                    net.linkIndicesOfStrands[springIdx]) &&
        ...
        for (long int crosslinkIdx = net.nrOfNodes - 1; crosslinkIdx >= 0;
          ...
          std::cout << "Removing f = 1 x-link " << crosslinkIdx << std::endl;
                ...
                this->removeSpringFollowingEntanglementLinks(
                    ...
                    net.strandIndicesOfLinks[crosslinkIdx][0]);
                numRemoved += 1;
    ...
    size_t MEHPForceBalance2::breakTooLongSprings(
        ...
        for (long int partialSpringIdx = net.nrOfSprings;
             partialSpringIdx >= 0;
             --partialSpringIdx) {
    ...
    void MEHPForceBalance2::removeSpring(ForceBalance2Network &net,
      ...
      std::cout << "Starting to remove spring " << springIdx << std::endl;
        ...
        net.strandIdxOfSpring -=
                (net.strandIdxOfSpring > springIdx).cast<int>();

        assert(net.springIndicesOfStrand.size() == net.nrOfStrands);
        for (size_t loopingSpringIdx = 0; loopingSpringIdx < net.nrOfStrands;
             ++loopingSpringIdx) {
            for (size_t i = 0;
                 i < net.springIndicesOfStrand[loopingSpringIdx].size();
                 ++i) {
                for (size_t partialSpringIdx: affectedPartialSprings) {
                    if (net.springIndicesOfStrand[loopingSpringIdx][i] >
                        partialSpringIdx) {
        ...
        for (size_t outermostI = 0; outermostI < linksToRemove.size();
            ...
            std::vector<size_t> springsOfLink =
                    net.strandIndicesOfLinks[slipLinkIdx];
            ...
            if (involvedPartialSprings.size() > 0) {
                ...
                // distance
                springIsAffected[net.strandIdxOfSpring[partialSpringToKeep]] =
                        true;
            }
    ...
    void MEHPForceBalance2::removeLink(ForceBalance2Network &net,
      ...
      std::cout << "Removing link " << linkIdx << std::endl;
        ...
        net.strandIndicesOfLinks.erase(net.strandIndicesOfLinks.begin() +
                                       linkIdx);
    ...
    void MEHPForceBalance2::mergePartialSprings(
        ...
        INVALIDARG_EXP_IFN(
            ...
            std::to_string(net.springIndexA[removedPartialSpringIdx]) +
            " and " +
      ...
      std::cout << "Merging partial springs " << removedPartialSpringIdx
        ...
        int found =
                std::erase(net.strandIndicesOfLinks[linkToReduce], fullSpringIdx);
        ...
        found = 0;
        for (int j = net.springIndicesOfStrand[fullSpringIdx].size() - 1;
             j >= 0;
             --j) {
        ...
        pylimer_tools::utils::removeRows(net.springCoordinateIndexB,
                                         ...
                                         3,
                                         skipEigenResize);
        pylimer_tools::utils::removeRows(net.springBoxOffset,
                                         3 * removedPartialSpringIdx,
                                         3,
                                         skipEigenResize);
        // renumber stuff
        for (size_t loopSpringIdx = 0;
             ...
             ++loopSpringIdx) {
            for (size_t i = 0;
                 i < net.springIndicesOfStrand[loopSpringIdx].size();
                 ++i) {
    ...
    void MEHPForceBalance2::mergeSprings(ForceBalance2Network &net,
        ...
        size_t fullSpringIdx = net.strandIdxOfSpring[keptSpringIdx];
        // handle links
        ...
        size_t remainingPartialSpringIdx =
                (keptSpringsLinks[keptSpringsLinks.size() - 1] == linkToReduce)
                    ? pylimer_tools::utils::last(
                        net.springIndicesOfStrand[keptSpringIdx])
        ...
        RUNTIME_EXP_IFN(
            net.springIndexA[remainingPartialSpringIdx] == linkToReduce ||
        ...
        if (keptSpringsLinks[keptSpringsLinks.size() - 1] == linkToReduce) {
            ...
            if (removedSpringsLinks[removedSpringsLinks.size() - 1] ==
                ...
                for (int i =
                             net.springIndicesOfStrand[removedSpringIdx].size() - 2;
                     i >= 0;
                     --i) {
                ...
                for (size_t partialSpringIdxToInvert:
                    ...
                    for (int dir = 0; dir < 3; ++dir) {
                        std::swap(
                            net.springCoordinateIndexA[3 * partialSpringIdxToInvert +
                                                       dir],
                            net.springCoordinateIndexB[3 * partialSpringIdxToInvert +
                                                       dir]);
                    }
                    net.springBoxOffset.segment(3 * partialSpringIdxToInvert, 3) *=
                            -1.;
                }
                ...
                for (size_t i = 1;
                     i < net.springIndicesOfStrand[removedSpringIdx].size();
                     ++i) {
                    net.springIndicesOfStrand[keptSpringIdx].push_back(
                        net.springIndicesOfStrand[removedSpringIdx][i]);
                }
        ...
        for (size_t linkOfRemovedSpring: removedSpringsLinks) {
            for (size_t i = 0;
                 i < net.strandIndicesOfLinks[linkOfRemovedSpring].size();
                 ++i) {
        ...
        for (int i = net.strandIndicesOfLinks[linkToReduce].size() - 1; i >= 0;
             --i) {
        ...
        if (net.springIndexA[remainingPartialSpringIdx] == linkToReduce) {
            net.springIndexA[remainingPartialSpringIdx] =
            ...
            RUNTIME_EXP_IFN(
                net.springIndexB[remainingPartialSpringIdx] == linkToReduce, "");
            net.springIndexB[remainingPartialSpringIdx] =
                    otherEndOfRemovedSpring;
            for (size_t dir = 0; dir < 3; ++dir) {
        ...
        net.springBoxOffset.segment(3 * remainingPartialSpringIdx, 3) +=
                offsetMultiplier *
                net.springBoxOffset.segment(3 * removedPartialSpringIdx, 3);
        pylimer_tools::utils::removeRow(net.springIndexA,
                                        removedPartialSpringIdx);
        pylimer_tools::utils::removeRow(net.springIndexB,
                                        removedPartialSpringIdx);
        pylimer_tools::utils::removeRows(
        ...
        for (size_t i = 0;
             i < net.springIndicesOfStrand[newKeptSpringIdx].size();
             ++i) {
            size_t partialSpringIdx =
                    net.springIndicesOfStrand[newKeptSpringIdx][i];
        ...
        for (long int crosslinkIdx = net.nrOfNodes - 1; crosslinkIdx >= 0;
            ...
            if (net.strandIndicesOfLinks[crosslinkIdx].size() == 2) {
                ...
                if (springsToMerge[0] != springsToMerge[1] &&
                    (XOR(net.linkIndicesOfStrands[springsToMerge[0]][0] ==
                         crosslinkIdx,
                         pylimer_tools::utils::last(
                             net.linkIndicesOfStrands[springsToMerge[0]]) ==
                         crosslinkIdx)) &&
                    (XOR(net.linkIndicesOfStrands[springsToMerge[1]][0] ==
                         crosslinkIdx,
                         pylimer_tools::utils::last(
    ...
    void MEHPForceBalance2::reAlignSlipLinkToImages(
        ...
        assert(net.springIndexB[partialSpringIdx1] == slipLinkIdx);
        assert(net.springIndexA[partialSpringIdx2] == slipLinkIdx);
        assert(net.linkIsEntanglement[slipLinkIdx]);
        assert(net.strandIdxOfSpring[partialSpringIdx1] ==
            net.strandIdxOfSpring[partialSpringIdx2]);
        ...
        Eigen::Vector3d sourceCoords =
                net.coordinates.segment(3 * net.springIndexA[partialSpringIdx1],
                                        3) +
                u.segment(3 * net.springIndexA[partialSpringIdx1], 3);
        Eigen::Vector3d targetCoords =
                net.coordinates.segment(3 * net.springIndexB[partialSpringIdx2],
                                        3) +
                u.segment(3 * net.springIndexB[partialSpringIdx2], 3);
        ...
        net.springBoxOffset.segment(3 * partialSpringIdx1, 3) = bestOffset;
        net.springBoxOffset.segment(3 * partialSpringIdx2, 3) =
    ...
    double MEHPForceBalance2::displaceToMeanPosition(
        ...
        nSpringsPerLink(net.springCoordinateIndexA) +=
                loopPartialSpringEliminator;
        nSpringsPerLink(net.springCoordinateIndexB) +=
                loopPartialSpringEliminator;
        nSpringsPerLink =
    ...
    double MEHPForceBalance2::displaceToMeanPosition(
        ...
        for (const size_t globalSpringIndex: partialSpringIndices) {
            ...
            double oneOverContourLengthFraction = 1. / net.springsContourLength[globalSpringIndex];
    ...
    Eigen::Matrix3d MEHPForceBalance2::evaluateStressOnLink(
        ...
        for (const size_t globalSpringIndex: partialSpringIndices) {
            Eigen::Vector3d partialDistance =
                    this->evaluatePartialSpringDistanceFrom(
                        net, u, globalSpringIndex, linkIdx);
            double oneOverContourLengthFraction = 1.0 / net.springsContourLength[globalSpringIndex];
    ...
    double MEHPForceBalance2::getAverageSpringLength() const {
        ...
        for (size_t i = 0; i < this->initialConfig.nrOfSprings; ++i) {
            springVectors.segment(this->initialConfig.strandIdxOfSpring[i],
    ...
    Eigen::VectorXd MEHPForceBalance2::evaluateSpringLengths(const ForceBalance2Network &net,
                                                             Eigen::VectorXd u) const {
        Eigen::VectorXd springVectors = this->evaluateSpringVectors(
            net, u);
        Eigen::VectorXd springLengths =
                Eigen::VectorXd::Zero(net.nrOfStrands);
        for (size_t i = 0; i < net.nrOfStrands; ++i) {
    ...
    Eigen::VectorXd MEHPForceBalance2::evaluateSpringVectors(const ForceBalance2Network &net,
                                                             Eigen::VectorXd u) const {
        Eigen::VectorXd partialSpringVectors = this->evaluatePartialSpringVectors(
            net, u);
        Eigen::VectorXd springVectors =
                Eigen::VectorXd::Zero(3 * net.nrOfStrands);
        for (size_t i = 0; i < net.nrOfSprings; ++i) {
            springVectors.segment(net.strandIdxOfSpring[i],
                                  3) += partialSpringVectors.segment(3 * i, 3);
        }
    ...
    MEHPForceBalance2::evaluateStressTensor(
        ...
        for (size_t partialSpringIdx = 0;
             partialSpringIdx < net.nrOfSprings;
             ++partialSpringIdx) {
            Eigen::Vector3d distance =
                    relevantPartialDistancesA.segment(3 * partialSpringIdx, 3);
            double oneOverContourLengthFraction = 1. / net.springsContourLength[partialSpringIdx];
    ...
    Eigen::VectorXd MEHPForceBalance2::getGammaFactors(
        ...
        for (size_t i = 0; i < springVectors.size() / 3; ++i) {
            double oneOverContourLengthFraction = 1.0 / this->initialConfig.springsContourLength[i];
            ...
            RUNTIME_EXP_IFN(
                ...
                ", computed from 1/N = " + std::to_string(oneOverContourLengthFraction) + ", b02 = " +
                std::to_string(b02) + ", and squared distance = " +
    ...
    Eigen::VectorXd MEHPForceBalance2::getGammaFactorsInDir(
        ...
        for (size_t i = 0; i < springVectors.size() / 3; ++i) {
            ...
            RUNTIME_EXP_IFN(std::isfinite(gammaFactors[i]),
                            ...
                            std::to_string(i) +
                            ", computed from 1/N = " + std::to_string(oneOverContourLengthFraction) +
    ...
    bool MEHPForceBalance2::validateNetwork(const ForceBalance2Network &net,
        ...
        RUNTIME_EXP_IFN(net.springIndicesOfStrand.size() == net.nrOfStrands,
                        ...
                        std::to_string(net.springIndicesOfStrand.size()) +
                        " for " + std::to_string(net.nrOfStrands) +
                        " springs.");
        RUNTIME_EXP_IFN(net.springsContourLength.size() == net.nrOfStrands,
        ...
        RUNTIME_EXP_IFN(net.strandIndicesOfLinks.size() == net.nrOfLinks,
                        ...
                        std::to_string(net.linkIndicesOfStrands.size()) +
                        " for " + std::to_string(net.nrOfStrands) +
                        " springs.");
        ...
        RUNTIME_EXP_IFN(net.springBoxOffset.size() ==
                        net.nrOfSprings * 3,
        ...
        if (net.nrOfStrands > 0) {
            RUNTIME_EXP_IFN(
                net.strandIdxOfSpring.maxCoeff() < net.nrOfStrands,
                "Partial spring must map to full spring, which must have "
                "a lower index.");
            RUNTIME_EXP_IFN(net.springCoordinateIndexA.maxCoeff() <
                            3 * net.nrOfLinks,
                            "Part coordinates must map to coordinates.");
            RUNTIME_EXP_IFN(net.springCoordinateIndexB.maxCoeff() <
                            3 * net.nrOfLinks,
                            "Part coordinates must map to coordinates.");
        ...
        for (size_t link_idx = 0; link_idx < net.nrOfLinks; ++link_idx) {
            ...
            std::vector<size_t> thisLinksSprings =
                    net.strandIndicesOfLinks[link_idx];
         ...
         */
        for (size_t i = 0; i < net.nrOfSprings; i++) {
            size_t fullIdx = net.strandIdxOfSpring[i];
            size_t partialEndA = net.springIndexA[i];
            size_t partialEndB = net.springIndexB[i];
            ...
            for (int dir = 0; dir < 3; ++dir) {
                RUNTIME_EXP_IFN(
                    net.springCoordinateIndexA[3 * i + dir] ==
                    3 * partialEndA + dir,
                ...
                RUNTIME_EXP_IFN(
                    net.springCoordinateIndexB[3 * i + dir] ==
                    3 * partialEndB + dir,
                    "Spring part index and coordinate index must match. Got " +
