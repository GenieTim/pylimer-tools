...
namespace pylimer_tools::sim::mehp {
    MEHPForceBalance2::MEHPForceBalance2(
        ...
        assert(baseCoordinates.size() == u.getNrOfAtoms() * 3);
            ...
            while (!vertexStack.empty()) {
                ...
                for (igraph_integer_t i = 0; i < igraph_vector_int_size(neighbors); ++i) {
                    ...
                    if (vertex_visited[neighbor] && edge_followed[edge]) {
                        ...
                        edge_followed[edge] = true;
                        // compute box offset
                        Eigen::Vector3d distance = -baseCoordinates.segment(
                                                       currentVertex * 3, 3
                                                   ) - baseCoordinates.segment(
                                                       newLinkIdxToOldVertexIdx[springTo.back()] * 3, 3
                                                   );
        ...
        this->initialConfig.springIsEntanglement = Eigen::ArrayXb::Constant(this->initialConfig.nrOfSprings, false);
        if (entanglementsAsSprings) {
        ...
        this->initialConfig.strandIndicesOfLink = pylimer_tools::utils::initializeWithValue(
            this->initialConfig.nrOfLinks, std::vector<size_t>());
        ...
        for (size_t linkIdx = 0; linkIdx < nextLinkIdx; ++linkIdx) {
            ...
            this->initialConfig.coordinates.segment(3 * linkIdx, 3) =
                    baseCoordinates.segment(3 * newLinkIdxToOldVertexIdx[linkIdx], 3);
            this->initialConfig.linkIsEntanglement(linkIdx) = pairOfAtom[newLinkIdxToOldVertexIdx[linkIdx]] >= 0;
        }

        this->initialConfig.nrOfNodes = (this->initialConfig.linkIsEntanglement == false).count();
        ...
        if (!entanglementsAsSprings) {
            ...
            assert(this->initialConfig.springContourLength.sum() ==
                this->universe.getNrOfBonds() + this->initialConfig.springIsEntanglement.count());
        }
    ...
    MEHPForceBalance2::removeInactiveCrosslinks(ForceBalance2Network &net,
      ...
      std::cout << "Removing f = 1 x-link " << crosslinkIdx << std::endl;
                ...
                this->removeStrandFollowingEntanglementLinks(
                    net, displacements, net.strandIndicesOfLink[crosslinkIdx][0]);
                numRemoved += 1;
  ...
  std::cout << "Removed spring " << springIdx << std::endl;
    ...
    MEHPForceBalance2::removeStrandFollowingEntanglementLinks(
      ...
      std::cout << "Removing additional spring between entanglement links "
                ...
                this->removeStrand(
                    net, displacements, net.strandIndicesOfLink[linkIdxToDelete][0]);
            }
