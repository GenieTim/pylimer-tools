    ...
    namespace sim::mehp {
...
#ifdef CEREALIZABLE
            MEHPForceBalance2() {
            }; // not exposed to users, only used by Cereal
        ...
        public:
            ...
            static MEHPForceBalance2 constructWithSlipLinks(
                ...
                fb.initialConfig.springCoordinateIndexB.conservativeResize(
                    3 * numPartialSpringsEstimate);
                fb.initialConfig.springIndexA.conservativeResize(
                    numPartialSpringsEstimate);
                fb.initialConfig.springIndexB.conservativeResize(
                    numPartialSpringsEstimate);
            ...
            Eigen::VectorXd evaluateSpringLengths(const ForceBalance2Network &net, Eigen::VectorXd u) const;

            Eigen::VectorXd evaluateSpringVectors(const ForceBalance2Network &net, Eigen::VectorXd u) const;
            ...
            void deformTo(pylimer_tools::entities::Box &newBox) {
                ...
                this->box.adjustCoordinatesTo(this->initialConfig.springBoxOffset,
                                              newBox);
                this->box = newBox;
            ...
            std::vector<double> getOverallSpringLengths() const {
                ...
                assert(partialSpringDistances.size() ==
                    this->initialConfig.nrOfSprings);
            ...
            double sumToTotalFraction(const ForceBalance2Network &net,
                ...
                double alpha = 0.;
                for (size_t i = 0; i < net.springIndicesOfStrand[springIdx].size();
                     ++i) {
            ...
            Eigen::Vector3d getForceOn(
                ...
                return this->evaluateForceOnLink(
                    index, net, u, debugNrSpringsVisited);
            }
        ...
        protected:
            ...
            void setPartialSpringPropertiesBasedOnChain(
                ...
                size_t from = net.springIndexA[partialSpringIdx];
                size_t to = net.springIndexB[partialSpringIdx];
                ...
                net.strandIdxOfSpring[partialSpringIdx] = springIdx;
                ...
                net.springBoxOffset.segment(3 * partialSpringIdx, 3) =
                        Eigen::Vector3d::Zero();
            ...
            size_t getOtherRailPartialSpringIdx(
                ...
                size_t fullSpringIdx = net.strandIdxOfSpring[partialSpringIdx];

                for (size_t i = 0; i < net.springIndicesOfStrand[fullSpringIdx].size();
                     ++i) {
                    if (net.springIndicesOfStrand[fullSpringIdx][i] ==
                        partialSpringIdx) {
                        if (i == 0) {
                            return net.springIndicesOfStrand[fullSpringIdx][i + 1];
                        }
                        if (i >= (net.springIndicesOfStrand[fullSpringIdx].size() - 1)) {
                            return net.springIndicesOfStrand[fullSpringIdx][i - 1];
                        }
                        size_t candidate1 =
                                net.springIndicesOfStrand[fullSpringIdx][i - 1];
                        size_t candidate2 =
                                net.springIndicesOfStrand[fullSpringIdx][i + 1];
                        ...
                        if (this->isPartOfSpring(net, aroundLinkIdx, candidate1) &&
                            ...
                            net.springIndexA[partialSpringIdx] !=
                            net.springIndexB[partialSpringIdx]) {
                            // this is a b-a-b-a situation
                            // check the ordered linkIndicesOfSprings
                            if (net.linkIndicesOfStrands[fullSpringIdx][i] == aroundLinkIdx) {
                                return candidate1;
                            } else {
                                assert(net.linkIndicesOfStrands[fullSpringIdx][i + 1] ==
                                    aroundLinkIdx);
                                return candidate2;
                            }
                        }
            ...
            bool isPartOfSpring(const ForceBalance2Network &net,
                                ...
                                size_t partialSpringIdx) const {
                return (net.springIndexA[partialSpringIdx] == linkIdx) ||
                       (net.springIndexB[partialSpringIdx] == linkIdx);
            }

            bool isLoopingSpring(const ForceBalance2Network &net,
                                 size_t partialSpringIdx) const {
                return (net.springIndexA[partialSpringIdx] ==
                        net.springIndexB[partialSpringIdx]);
            }
