...
void UniverseSequence::initializeFromDumpFile(
  ...
  pylimer_tools::utils::DumpFileParser dumpFileParser =
      pylimer_tools::utils::DumpFileParser();
...
#pragma omp for
  for (size_t i = 0; i < nrOfTimesteps; ++i) {
    ...
    newUniverse.setTimestep(
        dumpFileParser.getValuesForAt<long int>(i, "TIMESTEP", 0)[0]);
    if (dumpFileParser.hasKey("BOX BOUNDS")) {
      std::vector<double> lo =
          dumpFileParser.getValuesForAt<double>(i, "BOX BOUNDS", 0);
      std::vector<double> hi =
          dumpFileParser.getValuesForAt<double>(i, "BOX BOUNDS", 1);
      newUniverse.setBoxLengths(hi[0] - lo[0], hi[1] - lo[1], hi[2] - lo[2]);
    } else {
      newUniverse.setBoxLengths(dataFileParser.getLx(), dataFileParser.getLy(),
                                dataFileParser.getLz());
    }
    ...
    std::vector<double> positionsX =
        dumpFileParser.getValuesForAt<double>(i, "ATOMS", "x" + positionSuffix);
    std::vector<double> positionsY =
        dumpFileParser.getValuesForAt<double>(i, "ATOMS", "y" + positionSuffix);
    std::vector<double> positionsZ =
        dumpFileParser.getValuesForAt<double>(i, "ATOMS", "z" + positionSuffix);
    ...
    if (dumpFileParser.hasKey("NUMBER OF ATOMS")) {
      std::vector<int> nAtomVec =
          dumpFileParser.getValuesForAt<int>(i, "NUMBER OF ATOMS", 0);
      if (nAtomVec.size() > 0) {
        nAtoms = nAtomVec[0];
      }
    }
    ...
    if (dumpFileParser.keyHasDirectionalColumn("ATOMS", "i", "")) {
      ...
      nx = pylimer_tools::utils::initializeWithValue(
          nAtoms, 0); // dataFileParser.getAtomNx();
      ny = pylimer_tools::utils::initializeWithValue(
          nAtoms, 0); // dataFileParser.getAtomNy();
      nz = pylimer_tools::utils::initializeWithValue(
          nAtoms, 0); // dataFileParser.getAtomNz();
    }
    ...
    if (dumpFileParser.keyHasColumn("ATOMS", "type")) {
      ...
      if (hasAtomIds) {
        ...
        Universe dataFileUniverse =
            Universe(dataFileParser.getLx(), dataFileParser.getLy(),
                     dataFileParser.getLz());
        dataFileUniverse.addAtoms(
            dataFileParser.getNrOfAtoms(), dataFileParser.getAtomIds(),
            dataFileParser.getAtomTypes(), dataFileParser.getAtomX(),
            dataFileParser.getAtomY(), dataFileParser.getAtomZ(),
            dataFileParser.getAtomNx(), dataFileParser.getAtomNy(),
            dataFileParser.getAtomNz());
        for (long int j = 0; j < nAtoms; ++j) {
    ...
    newUniverse.addBonds(
        dataFileParser.getNrOfBonds(), dataFileParser.getBondFrom(),
        dataFileParser.getBondTo(), dataFileParser.getBondTypes(), true);
...
Universe UniverseSequence::atIndex(int index) {
  ...
  if (!this->universeCache.contains(index)) {
    this->universeCache.emplace(index,
                                this->readDataFile(this->dataFiles[index]));
  }
