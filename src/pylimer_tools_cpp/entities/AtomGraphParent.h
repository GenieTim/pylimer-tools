...
namespace entities {
  ...
  public:
     ...
     * scope.
     *
     * @return std::unique_ptr<igraph_t, std::function<void(igraph_t*)>> A
     * unique_ptr to a copy of the graph
     */
    std::unique_ptr<igraph_t> getCopyOfGraph() const
    {
      igraph_t graphCopy;
      igraph_copy(&graphCopy, &this->graph);
      return std::unique_ptr<igraph_t>(&graphCopy);
    }
