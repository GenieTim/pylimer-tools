...
namespace entities {
   ...
   */
  Eigen::ArrayXi
  getIndicesCloseToCoordinates(const Eigen::Vector3d& coordinates,
                               const double upperCutoff) const
  {
    ...
    if (upperCutoff == this->cutoff) {
      ...
      bucketIndices =
        this->getCombinedBucketIndicesForCoordinates(coordinates, upperCutoff);
    }
...
#ifndef NDEBUG
    Eigen::Array3i verifyCoeffs = this->getBucketTripletForCoordinates(results);
...
private:
  ...
  std::vector<std::vector<bucket_idx_t>> neighbourBucketNeighboursDefaultCutoff;
