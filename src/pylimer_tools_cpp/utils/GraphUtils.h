...
namespace utils {
  ...
  public:
    ...
    std::vector<long int> findNext()
    {
      ...
      while (this->currentStartIndex < this->startingIndices.size()) {
        ...
        while (this->currentNeighbourIndex < igraph_vector_size(&neighbours)) {
          long int neighbourVertexId =
            igraph_vector_e(&neighbours, this->currentNeighbourIndex);
          ...
          // find the loops
          igraph_vector_t verticesOfLoop;
          igraph_vector_init(&verticesOfLoop, 0);
          ...
          if (igraph_vector_size(&verticesOfLoop) > 0) {
            // assemble it and its hash
            long int loopHash = 0;
            std::vector<long int> results;
            results.reserve(igraph_vector_size(&verticesOfLoop));
            for (size_t i = 0; i < igraph_vector_size(&verticesOfLoop); ++i) {
              long int loopVertexId = igraph_vector_e(&verticesOfLoop, i);
              loopHash =
            ...
            if (!this->loopsFound.contains(loopHash)) {
              ...
              igraph_vector_destroy(&verticesOfLoop);
  ...
  protected:
    ...
    void findStartingIndices(int minDegree = 2)
    {
      ...
      igraph_vector_init(&degrees, igraph_vcount(&this->graph));
      ...
      for (size_t i = 0; i < igraph_vcount(&this->graph); ++i) {
        if (igraph_vector_e(&degrees, i) > minDegree) {
      ...
      if (minDegree >= 2) {
        ...
        for (size_t i = 0; i < NComponents; ++i) {
          ...
          if (maxDegree <= minDegree && maxDegree > 1) {
            // TODO: here, add an index of the new cluster but its index as
            // found in the old graph
