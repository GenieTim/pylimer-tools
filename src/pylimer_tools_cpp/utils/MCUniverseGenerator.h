...
public:
  ...
  void removeCrosslink(size_t crosslinkIdx)
  {
    ...
    for (size_t i = 0; i < this->simplifiedUniverse.strandFrom.size(); ++i) {
      INVALIDARG_EXP_IFN(this->simplifiedUniverse.strandFrom[i] !=
                             crosslinkIdx &&
    ...
    if (dist.norm() < maxDistance || maxDistance < 0.) {
      ...
      if (this->secondaryLoopProbability != 1.) {
        ...
        assert(xlinkIdxOnStrand1 <
               this->simplifiedUniverse.xlinkChainId.size());
        while (
          xlinkIdxOnStrand1 < this->simplifiedUniverse.xlinkChainId.size() &&
          this->simplifiedUniverse.xlinkChainId[xlinkIdxOnStrand1] == strand1) {
