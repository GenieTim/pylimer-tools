...
def compute_weight_fraction_of_soluble_material_from_weight_fractions(r: float, p: float, f: int,
                                                                      w_f: float, w_g: float, g: int = 2):
    """
...
def compute_weight_fractions_and_probabilities(network: Universe, crossLinker_type: int,
    ...
    if (weight_fractions is None):
        ...
        assert (math.isclose(
            sum(w for w in weight_fractions.values()), 1., abs_tol=1e-9))
...
def compute_miller_macosko_probabilities(r: float, p: float, f: int):
    ...
    if (beta > 1 or beta < 0):
        warnings.warn(
            "The resulting P(F_B) from r = {}, p = {} for f = {} is probably unreliable, ".format(r, p, f) +
...
def compute_extracted_modulus(p: float, r: float, f: int, g_e_1: pint.Quantity, w_sol: float,
                              xlink_concentration_0: pint.Quantity,
                              alpha: Union[float, None] = None, temperature: pint.Quantity = None,
...
def compute_entanglement_modulus(p: float, r: float, f: int, g_e_1: pint.Quantity,
                                 alpha: Union[float, None] = None, temperature: pint.Quantity = None,
                                 unit_style: Union[None, UnitStyle] = None):
    """
...
def compute_junction_modulus(p: float, r: float, xlink_concentration_0: pint.Quantity, unit_style: UnitStyle,
                             f: Union[int, None] = None, alpha: Union[float, None] = None,
...
def compute_probability_that_monomer_is_effective(functionality_of_monomer: int, expected_degree_of_effect: int,
                                                  p_f_a_out: float):
    """
