...
print(
    f"Linking strand ends to achieve {strand_to_strand_conversion:.1%} conversion...")
...
print(f"  Total bonds: {generator.get_current_nr_of_bonds()}")
print(
    f"  Final strand conversion: {generator.get_current_strand_conversion():.3f}")
...
print(
    f"  Box dimensions: {box.get_lx():.1f} × {box.get_ly():.1f} × {box.get_lz():.1f}")
