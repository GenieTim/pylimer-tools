    ...
    SECTION("MEHP Force Balance2 3D case") {
        ...
        if (std::filesystem::exists(largeInputFile)) {
            ...
            SECTION("Actual balance results in correct phantom results") {
                ...
                CHECK_THAT(forceRelaxer.getResidual(), Catch::Matchers::WithinAbs(forceBalancer2.getResidual(), 1e-5));
...
TEST_CASE("MEHPFB2 Basic conversion test", "[analysis][MEHPForceBalance2][MEHPForceBalance]") {
    ...
    std::vector<double> coordsX = {
        ...
        7., 9., 30., 11., -7., // 11-15: diagonal line
    };
    ...
    std::vector<double> coordsZ = {
        ...
        7., 9., 10., -19., 3., // 11-15: diagonal line
    };
    ...
    std::vector<long int> bondFrom = {
        0, 1, 2, 3, 4, 5, 0, 6, 7, 8, 9, 10, 0, 11, 12, 13, 14, 15, 2
    };
    std::vector<long int> bondTo = {
        1, 2, 3, 4, 5, 0, 6, 7, 8, 9, 10, 0, 11, 12, 13, 14, 15, 0, 7
    };
    universe.addBonds(
        bondFrom,
        bondTo
    ...
    pylimer_tools::sim::mehp::MEHPForceBalance fb1 = pylimer_tools::sim::mehp::MEHPForceBalance(universe);
    fb1.configAssumeBoxLargeEnough(false);

    pylimer_tools::sim::mehp::MEHPForceBalance2 fb2 = pylimer_tools::sim::mehp::MEHPForceBalance2(universe);

    pylimer_tools::sim::mehp::ForceBalance2Network net = fb2.getNetwork();

    CHECK(net.nrOfLinks == 1);
    CHECK(net.nrOfSprings == 3);
    CHECK(net.nrOfStrands == 3);
    CHECK(net.nrOfNodes == 1);
    CHECK_FALSE(net.springBoxOffset.isZero());

    CHECK_THAT(fb1.getResidual(), Catch::Matchers::WithinRel(fb2.getResidual(), 1e-12));

    fb1.runForceRelaxation();
    fb2.runForceRelaxation();

    CHECK_THAT(fb1.getResidual(), Catch::Matchers::WithinAbs(fb2.getResidual(), 1e-12));
    CHECK_THAT(fb1.getGamma(), Catch::Matchers::WithinAbs(fb2.getGamma(), 1e-12));
}
