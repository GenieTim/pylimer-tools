...
TEST_CASE("MEHP Force Relaxation2 runs", "[analysis][MEHPForceRelaxation2]")
{
  ...
  SECTION("3D case")
  {
    universeSeq.initializeFromDataSequence(
      { { suspectedPath +
          "xlinked_0.90015_pdms_1e4_a_78_bs_t_807536.structure.out" } });
    ...
    REQUIRE(forceRelaxer2.getVolume() == Catch::Approx(universe2.getVolume()));
    ...
    REQUIRE(conversionFactor == Catch::Approx(0.000245543/(sigmaToNm*sigmaToNm)));
    ...
    REQUIRE(forceRelaxer2.getFinalPressure()/conversionFactor == Catch::Approx(61308.3));
  ...
  SECTION("2D case")
  {
    ...
    // forceRelaxer.runForceRelaxation(5, 1e-5, 15, true, 0.075);
